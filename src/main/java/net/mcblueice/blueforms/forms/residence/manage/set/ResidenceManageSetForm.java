package net.mcblueice.blueforms.forms.residence.manage.set;
...
public class ResidenceManageSetForm {
	...
	public ResidenceManageSetForm(Player player, ConfigManager lang, ClaimedResidence claimedResidence) {
		this.player = player;
		this.lang = lang;
	...
	public void open() {
		...
		if (!ResidenceUtils.canManage(player, claimedResidence)) {
			player.sendMessage(lang.get("prefix") + lang.get("forms.residence.manage.set.nopermissionmessage", claimedResidence.getName()));
			return;
		}
		...
		CustomForm.Builder builder = CustomForm.builder()
			.title(lang.get("forms.residence.manage.set.title", claimedResidence.getName()));
		...
		builder.validResultHandler((form, response) -> {
			for (int i = 0; i < permissionList.size(); i++) {
				...
				if (newState != oldState) {
					claimedResidence.getPermissions().setFlag(flagName, newState);
				}
			}
